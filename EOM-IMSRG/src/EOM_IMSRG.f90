...
subroutine calculate_excited_states(J,PAR,Numstates,HS,jbas,ladder_ops) 
 ...
 ! end if
  ...
  print*
  write(*,'((A26),(I1),(A3),(I1))')'EXCITED STATE ENERGIES: J=',J/2,' P=',PAR
  print*, '=============================================='
  ...
  open(unit=72,file=trim(OUTPUT_DIR)//&
       trim(adjustl(prefix))//&
       '_EOM_spec_law'//trim(betalabel)//'.dat',position='append')  
  do i = 1, Numstates
     write(72,'((I3),1X,(A1),3(f16.9))') J/2, Plabel, &
       ladder_ops(i)%E0,ladder_ops(i)%E0+HS%E0,sum(ladder_ops(i)%fph**2)
  end do
...
!  tps=0
  nTDA=sps+tps
  if(nTDA==sps) write(*,*) 'nTDA: ',nTDA,"  This is TDA calculations ..."
!hu TDA 
  ...
  tol = 1.0E-5 ! error tolerance? (wtf zero?) 
  if(NEV == 1) tol = 100
  info = 0
