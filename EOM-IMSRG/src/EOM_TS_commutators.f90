...
subroutine EOM_TS_commutator_121(L,R,RES,jbas) 
  ...
  rank = R%rank 
 !dfph~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  !$OMP PARALLEL DO DEFAULT(FIRSTPRIVATE), SHARED(L,R,RES)
...
! dfph ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  !$OMP PARALLEL DO DEFAULT(FIRSTPRIVATE), SHARED(LCC,R,RES)
...
subroutine EOM_TS_commutator_122(L,R,RES,jbas) 
  ...
  logical :: square
  real(8) ::  sm
  integer :: thread,total_threads


  total_threads = size(R%direct_omp) - 1
  
!$OMP PARALLEL DO DEFAULT(FIRSTPRIVATE), SHARED(L,R,RES)  
  do thread = 1, total_threads
  do q = 1+R%direct_omp(thread),R%direct_omp(thread+1)  
...
subroutine EOM_TS_commutator_221(w1,w2,pm,RES,jbas) 
  ...
  ! fph
  !$OMP PARALLEL DO DEFAULT(FIRSTPRIVATE), SHARED(w1,w2,RES)
  do ik = 1 , Ntot-Abody
     i = jbas%parts(ik) 
     ji = jbas%jj(i) 
     li = jbas%ll(i) 
...
!$OMP PARALLEL DO DEFAULT(FIRSTPRIVATE), SHARED(L,R,RES,w1,w2)  
  do thread = 1, total_threads
  do q = 1+R%direct_omp(thread),R%direct_omp(thread+1)  
...
!$OMP PARALLEL DO DEFAULT(FIRSTPRIVATE), SHARED(RES,WCC,RCC)  
  do thread = 1, total_threads
