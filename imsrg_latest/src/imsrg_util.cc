...
namespace imsrg_util
{
 ...
 Operator OperatorFromString(ModelSpace& modelspace, std::string opname )
 {
      ...
      else if (opname == "IVM")           theop =  MultipoleResponseOp(modelspace,2,0,1) ; // Isovector monopole (see PRC97(2018)054306 ) --added by bhu
      else if (opname == "ISQ")           theop =  MultipoleResponseOp(modelspace,2,2,0) ; // Isoscalar quadrupole (see PRC97(2018)054306 ) --added by bhu
      else if (opname == "IVQ")           theop =  MultipoleResponseOp(modelspace,2,2,1) ; // Isosvector quadrupole (see PRC97(2018)054306 ) --added by bhu
      else if (opname == "ISO")           theop =  MultipoleResponseOp(modelspace,3,3,0) ; // Isoscalar octupole (see PRC97(2018)054306 ) --added by bhu
      else if (opname == "IVO")           theop =  MultipoleResponseOp(modelspace,3,3,1) ; // Isosvector octupole (see PRC97(2018)054306 ) --added by bhu
      ...
      else if (opnamesplit[0] == "DMSDEFT") // Dark matter spin-dependent EFT operators
      {

        int L;
        double rho, c3, c4, cD, p;
        std::string dmopname = opnamesplit[1];
        std::istringstream(opnamesplit[2]) >> rho;
        std::istringstream(opnamesplit[3]) >> c3;
        std::istringstream(opnamesplit[4]) >> c4;
        std::istringstream(opnamesplit[5]) >> cD;
        std::string sfact_type = opnamesplit[6];
        std::istringstream(opnamesplit[7]) >> L;
        std::istringstream(opnamesplit[8]) >> p;

        rho = rho*pow(HBARC, 3);
        c3 = c3*pow(10.0, -3.0);
        c4 = c4*pow(10.0, -3.0);

        std::map<std::string, Operator (*)(ModelSpace&, double, double, double, double, std::string, int, double) > dmop = {
              {"Long5",   &DM_NREFT::Longitudinal_5},
              {"ET",   &DM_NREFT::Electric_Transverse_5},
              {"MT",   &DM_NREFT::Magnetic_Transverse_5},
             };
        if ( dmop.find(dmopname) != dmop.end() )
        {
          theop = dmop[dmopname](modelspace, rho, c3, c4, cD, sfact_type, L, p);
        }
      }
