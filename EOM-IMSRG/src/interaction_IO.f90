...
!==================================================================
!==================================================================
subroutine read_operator_human(op,jbas)
  implicit none

  integer :: a,b,x1,x2,c1,c2
  integer :: ihu,jrank,prank,trank,ist,T1,T2,P1,P2
  type(spd) :: jbas
  type(sq_op) :: op
  character(13) :: text
  integer :: c,d,J1,J2,rank,T,P,q,qx,N,J1x,J2x
  ...
  write(*,*) "Read operator with Human format: "//trim(adjustl(spfile))
!  open(unit=111,file=trim(SP_DIR)//trim(adjustl(spfile)))
  open(unit=111,file=trim(adjustl(spfile)))
  c=0
  do
  read(111,*) text
  text=trim(adjustl(text))
  if( text(1:9)=='$TwoBody:' ) exit
  c = c + 1
  end do
  c = c - 4
  rewind(111)
  ...
  if( a /= op%herm ) stop 'input Herm error in read_operator_human'
  read(111,*) jrank, trank, prank
  if( jrank*2 /= op%rank ) stop 'input Jrank error in read_operator_human'
  if( prank*2 /= op%dpar ) stop 'input Parity error in read_operator_human'
  read(111,*) text, V
  op%E0 = V
  read(111,*) text
  ...
  select case(c1+c2)
     case(0)
        op%fpp(a-hb4(a),b-hb4(b)) = V
        op%fpp(b-hb4(b),a-hb4(a)) = V * op%herm
     case(1)
        if (c1 > c2) then
           op%fph(b-hb4(b),a-pb4(a)) = V * &
              op%herm * (-1)**( (jbas%jj(a) - jbas%jj(b))/2 )
        else
           op%fph(a-hb4(a),b-pb4(b)) = V
        end if
     case(2)
        op%fhh(a-pb4(a),b-pb4(b)) = V
        op%fhh(b-hb4(b),a-hb4(a)) = V * op%herm
  end select
  end do
  ...
  read(111,*,iostat=ist) J1x,P1,T1,J2x,P2,T2,ax,bx,cx,dx,V
  if (ist > 0) STOP 'operator_human file error'
  if (ist < 0) exit
  ...
  T = (ta + tb)/2
  if( .not.(triangle(J1,J2,rank))) stop &
      'input rank error in read_operator_human'
!bhu  if( P /= P1 ) stop 'input P1 error in read_operator_human'
  if( T /= T1 ) stop 'input T1 error in read_operator_human'
...
!     if (c == d) pre = pre * sqrt( 2.d0 )
  ...
  ! in the rank zero qn storage
   ...
   op%tblck(q)%tgam(qx)%X(i1,i2) = V * pre * phase

  end do

  end do

end subroutine read_operator_human
!==================================================================
!==================================================================
subroutine read_tokyo_orbits(jbas,hp,hn,trips,jbx)
  ...
  integer :: q,r,r1,r2,Jtarget,PARtarget,jx
  real(8) :: e,hocc
  character(1) :: trips
  character(13) :: text

  interm= adjustl(intfile)
write(*,*) interm
  open(unit=39,file=trim(interm))
  ...
  read(39,*) ix, jx, hp, hn
  ix = ix + jx
  write(*,*) 'The total number of orbits: ', ix
  jx = ix
  ...
  do ix = 1,jx
     ...
     read(39,*) label, ni, li, ji, tzi, hocc
...
end subroutine read_tokyo_orbits
!==================================================================
!==================================================================
subroutine read_tokyo_hamiltonian(H,jbas)
  implicit none

  integer :: ist,Ntot,Tz,Par,J2,a,b,c,d,ihu,Nhu
  integer :: x,q,qx,cx,C1,C2,i1,i2,j_min,pre
  real(8) :: V
  type(spd) :: jbas
  type(sq_op) :: H
  character(17) :: text

  open(unit=110,file=trim(adjustl(intfile)))
  ...
  read(110,*)
  read(110,*)
  read(110,'(a17,e14.6)') text, V 
  write(*,*) 'input ZeroBody part of Hamiltonian is: ',V
  H%E0 = V
  read(110,*)
  read(110,*)
  read(110,*)
  ...
  do ihu = 1, Nhu
    ...
    a = a + 1
    b = b + 1
    if ( (a>Ntot) .or. (b>Ntot) ) cycle
    c1 = jbas%con(a)
    c2 = jbas%con(b)
    cx = c1+c2
    select case (cx)
      case(0)
        H%fpp(a-hb4(a),b-hb4(b)) = V
        H%fpp(b-hb4(b),a-hb4(a)) = V
      case(1)
        if (c2 > c1) then
          H%fph(a-hb4(a),b-pb4(b)) = V
        else
          H%fph(b-hb4(b),a-pb4(a)) = V
        end if
      case(2)
        H%fhh(a-pb4(a),b-pb4(b)) = V
        H%fhh(b-pb4(b),a-pb4(a)) = V
    end select
  end do

  read(110,*) Nhu, ihu, V
  do ihu = 1, Nhu
    read(110,*) a,b,c,d,J2,V
    a = a + 1
    b = b + 1
    c = c + 1
    d = d + 1
    J2 = J2 * 2
    Tz=( jbas%itzp(a)+jbas%itzp(b) )/2
    Par=( 1-(-1)**( jbas%ll(a)+jbas%ll(b) ) )/2
    if( (a>Ntot) .or. (b>Ntot) .or. (c>Ntot) .or. (d>Ntot) ) cycle

    q = block_index(J2,Tz,Par)
    C1 = jbas%con(a)+jbas%con(b) + 1 !ph nature
    C2 = jbas%con(c)+jbas%con(d) + 1
    ...
    if ( a > b )  then
      ...
      i1 = jbas%xmap(x)%Z( (J2-j_min)/2 + 2)
      pre = (-1)**( 1 + (jbas%jj(a) + jbas%jj(b) -J2)/2 )
    else
!      if (a == b) pre = pre * sqrt( 2.d0 )
...
!      if (c == d) pre = pre * sqrt( 2.d0 )
  ...
  close(110)
end subroutine read_tokyo_hamiltonian
!==================================================================  
!==================================================================
