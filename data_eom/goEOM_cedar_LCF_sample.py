...
for A in [208]:
 ...
 for sample in [2460]:
 #for sample in [2007, 2460, 3448, 3373, 1141, 3319, 3098, 1429, 3895, 1172, 90, 723]:
 #for sample in [2125, 2245, 750, 1469, 1177, 493, 500, 3621, 606, 3480, 3260, 3813]:
 #for sample in [3105, 2411, 3350, 774, 2618, 4117, 922, 1173, 1802, 3472]:
...
#   for hw in [10,12,14,16]:
     ...
     ARGS['LECs'] = 'DeltaGO394_sample%d'%sample
#     ARGS['LECs'] = 'N2LO_sat'
#     ARGS['LECs'] = 'DNNLOgo'
#     ARGS['LECs'] = 'DNLOgo450'
...
### Some optional parameters that we probably want in the output name if we're using them
     ...
     initfile = jobname+'_LCF300_EOM.ini'
...
#     fx.write(jobname + '_TDA_' + ARGS['Operators'] + '\n') 
     fx.write(jobname + '_LCF300_' + ARGS['Operators'] + '\n') 
...
### Submit the job if we're running in batch mode, otherwise just run in the current shell
     if batch_mode==True:
      ...
      if BATCHSYS == 'PBS':
       ...
       sfile.write(FILECONTENT%(NTHREADS,jobname,time_request,jobname,mail_address,NTHREADS,NTHREADS,cmd))
       sfile.close()
