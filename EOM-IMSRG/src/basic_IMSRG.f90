...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  integer,public :: global_counter1=0,global_counter2=0,global_counter3=0,nTDA=0
...
subroutine print_time
  ...
  write(*,"(A7,I6.2,A1,I2.2,A1,I2.2)") "TIME: ",hours,":",mins,":",secs
  print*
...
!!! print out the name of the nucleus
    ...
    character(2),dimension(0:85) :: names 
    character(3) :: Astr
...
subroutine read_sp_basis(jbas,hp,hn,eMax,lmax,trips,jbx)
  ...
  open(unit=39,file=trim(adjustl(SP_DIR))//trim(interm))
  hk=interm(1:2)
...
character(5) function spec_not(i,jbas) 
  ...
  character(1) :: l_str
  character(2) :: j_str,n_str
  
  l_let = (/'s','p','d','f','g','h'/) 
  
  write(n_str,'(I2)') jbas%nn(i)
  write(j_str,'(I2)') jbas%jj(i) 
...
subroutine read_main_input_file(input,H,htype,HF,method,EXcalc,COM,R2RMS,obsname,&
  ...
  if( intfile(len(trim(intfile))-3:len(trim(intfile))) == '.int') then 
     ...
     ! Heiko format
!bhu     if (spfile(1:2) .ne. 'hk') then 
!bhu        STOP 'inconsistent interaction and sps files' 
!bhu     end if 
  end if 
  ...
  TBME_DIR = find_file('IMSRG_ME_FILES',trim(adjustl(intfile))) 
  SP_DIR = find_file('IMSRG_SP_FILES',trim(adjustl(spfile)))
...
!bhu           PRINT*, 'FILE NOT FOUND: ',trim(adjustl(file_name))
!bhu           print*, 'CHECK ENVIRONMENT VARIABLE: ',trim(adjustl(directory_name))
!bhu           STOP
        end if
     end if
     istart = iend+2
