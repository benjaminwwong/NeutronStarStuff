...
subroutine EOM_observables( ladder_ops, iso_ops, O1,HS, Hcm, trans, mom, eom_states , jbas)
  ...
  character(1) :: statlab
  real(8) :: temp1, temp2, temp3, Gamma_L, deltaE, fac1, fac2
  integer :: points_L
  real(8),allocatable,dimension(:) :: STRENGTHS_L,ENERGIES_L
  ...
  do q = 1, trans%num
     ...
     if (dTZ ==0 ) then 
        If (to_Ground) then
           ...
           temp3=0.d0
           do i = 1, states
            temp1 = temp1 + Energies(i)*strengths(i)
            temp2 = temp2 + strengths(i)
            if(abs(Energies(i))>0.01) temp3 = temp3 + fac1*strengths(i)/Energies(i)
           end do
           write(31,*) 'electric dipole polarizability: ', temp3
           write(31,*) 'm1, energy weight E*R sum:      ', temp1
           write(31,*) 'm0, R(E) sum:                   ', temp2
           write(31,*) 'centroid m1/m0:                 ', temp1/temp2
           write(31,'(2(a20),a40)')'E    ','R(E)    ','photodisintegration cross section'
           do i = 1, states
           write(31,'(3(f20.8))') Energies(i),strengths(i),fac2*Energies(i)*strengths(i)
           end do
           close(31)

           allocate(Strengths_L(points_L),Energies_L(points_L))
           open(unit=32,file=trim(OUTPUT_DIR)//trim(adjustl(prefix))//&
                '_Lorentzian_fold_'//trans%oper//'_'//trans%Jpi1(q)//'_'//trans%Jpi2(q)//'.dat', &
                form='formatted')
           temp1=0.d0
           temp2=0.d0
           temp3=0.d0
           do i = 1, points_L
             Energies_L(i) = (i-1) * deltaE
             E_out = 0.d0
             do j = 1, states
               E_out=E_out+0.5*Gamma_L/( (Energies_L(i)-Energies(j))**2 + 0.25*Gamma_L*Gamma_L ) / &
                 3.1415926 * strengths(j)
             end do
             Strengths_L(i) = E_out
             temp1 = temp1 + deltaE*Energies_L(i)*E_out
             temp2 = temp2 + deltaE*E_out
             if(abs(Energies_L(i))>0.01) temp3 = temp3 + fac1*deltaE*E_out/Energies_L(i)
           end do
           write(32,*) 'electric dipole polarizability: ', temp3
           write(32,*) 'm1, energy weight E*R sum:      ', temp1
           write(32,*) 'm0, R(E) sum:                   ', temp2
           write(32,*) 'centroid m1/m0:                 ', temp1/temp2
           write(32,'(2(a20),a40)')'E    ','R(E)    ','photodisintegration cross section'
           do i = 1, points_L
              write(32,'(3(f20.8))')Energies_L(i),Strengths_L(i),fac2*Energies_L(i)*Strengths_L(i)
           end do
           close(32)
           
           deallocate(Energies,strengths)  
           deallocate(Energies_L,strengths_L)  
